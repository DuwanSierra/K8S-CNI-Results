...
El propósito del recomendador es convertir un perfil de uso organizacional en una recomendación de CNI justificada y accionable, sin exigir al operador conocimiento previo del modelo matemático. La entrada es el perfil del usuario, capturado por el componente \texttt{GuidedProfileBuilder} mediante cinco preguntas guiadas que ponderan las prioridades del despliegue (rendimiento, latencia, eficiencia de recursos y seguridad). Las respuestas se transforman en un vector de pesos normalizado que suma la unidad, coherente con los factores de ponderación definidos en el modelo (\cref{sec:apb_ponderaciones}). Como alternativa, el componente \texttt{ProfileSelector} permite elegir uno de los perfiles base predefinidos.
...
El motor de decisión reside en \texttt{recommendationModel.js} y reproduce el modelo MCDA del diseño. Para cada CNI, cada métrica se normaliza a una escala común según sea de beneficio o de costo, se pondera con el vector de pesos del perfil y se agrega en un puntaje único que ordena el ranking. El diseño incorpora la restricción no compensatoria de seguridad (\cref{sec:apb_ponderaciones}): un CNI que no aplica Network Policies en su plano de datos no puede compensar esa carencia con un buen rendimiento, lo que penaliza su puntaje de seguridad. De este modo, la lógica de la interfaz no introduce criterios nuevos: aplica exactamente el mismo procedimiento formalizado en \cref{sec:apb_criterios,sec:apb_mcda}.
