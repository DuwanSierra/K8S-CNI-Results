...
\hline
Throughput Sender --- Gbps \newline\textit{($\uparrow$ mayor es mejor: más datos por segundo)} & 9.21 Gbps & 9.48 Gbps & 9.67 Gbps & 9.35 Gbps \\
\hline
Throughput Receiver --- Gbps \newline\textit{($\uparrow$ mayor es mejor: dato que recibe la aplicación destino)} & 9.18 Gbps & 9.45 Gbps & 9.64 Gbps & 9.31 Gbps \\
\hline
Retransmisiones TCP --- paquetes/sesión \newline\textit{($\downarrow$ menor es mejor: cada retransmisión es un paquete que llegó con error y debió reenviarse, consumiendo capacidad extra)} & 12.4 & 4.8 & 2.1 & 7.3 \\
\hline
Diferencia Sender$-$Receiver \newline\textit{(idealmente cercana a cero: refleja paquetes en tránsito o pérdidas)} & $\sim$0.03 Gbps & $\sim$0.03 Gbps & $\sim$0.03 Gbps & $\sim$0.04 Gbps \\
\hline
Variabilidad entre corridas ($\sigma$) \newline\textit{($\downarrow$ menor es mejor: un canal estable produce resultados consistentes y predecibles)} & $\pm$0.18 Gbps & $\pm$0.09 Gbps & $\pm$0.07 Gbps & $\pm$0.12 Gbps \\
\hline
...
En todas las filas de la Tabla~\ref{tab:latencia-resultados}, \textbf{un valor menor es mejor ($\downarrow$)}: menos milisegundos significa que el CNI establece conexiones más rápido. Los percentiles P95 y P99 son especialmente relevantes: P95 indica el tiempo de conexión que supera solo el 5\% más lento de las mediciones, y P99 el que supera el 1\% más lento. En sistemas de alta demanda estos valores determinan la experiencia del usuario en el peor caso --- un P99 elevado significa que una de cada 100 conexiones tarda considerablemente más que el promedio.
...
\hline
Latencia mínima --- ms \newline\textit{(mejor caso observado)} & 0.412 ms & 0.287 ms & 0.198 ms & 0.341 ms \\
\hline
Latencia promedio --- ms \newline\textit{(comportamiento típico)} & 0.687 ms & 0.431 ms & 0.312 ms & 0.512 ms \\
\hline
Latencia máxima --- ms \newline\textit{(peor caso observado)} & 2.341 ms & 0.891 ms & 0.543 ms & 1.124 ms \\
\hline
Jitter / Desviación estándar --- ms \newline\textit{(variabilidad: cuánto oscila la latencia entre conexiones)} & $\pm$0.389 ms & $\pm$0.142 ms & $\pm$0.087 ms & $\pm$0.201 ms \\
\hline
Percentil P95 --- ms \newline\textit{(el 95\% de las conexiones son más rápidas que este valor)} & 1.834 ms & 0.712 ms & 0.478 ms & 0.893 ms \\
\hline
Percentil P99 --- ms \newline\textit{(el 99\% de las conexiones son más rápidas; el 1\% restante define el peor caso para el usuario)} & 2.156 ms & 0.843 ms & 0.521 ms & 1.067 ms \\
\hline
\end{tabularx}
\end{table}
...
\hline
CPU bajo estrés --- millicores \newline\textit{($\downarrow$ menor es mejor: menos CPU consume el CNI, más queda para las apps)} & 187 m & 143 m & 98 m & 162 m \\
\hline
CPU en reposo --- millicores \newline\textit{(costo fijo del agente cuando no hay tráfico activo)} & 12 m & 28 m & 31 m & 22 m \\
\hline
RAM RSS bajo estrés --- MiB \newline\textit{($\downarrow$ menor es mejor: memoria real ocupada bajo carga máxima)} & 42 MiB & 78 MiB & 185 MiB & 94 MiB \\
\hline
RAM RSS en reposo --- MiB \newline\textit{(huella de memoria base del agente sin tráfico)} & 31 MiB & 61 MiB & 168 MiB & 76 MiB \\
\hline
Eficiencia: Gbps/100\,m de CPU \newline\textit{($\uparrow$ mayor es mejor: más tráfico procesado por unidad de CPU consumida)} & 4.92 Gbps & 6.61 Gbps & 9.84 Gbps & 5.77 Gbps \\
\hline
...
Los resultados revelan un compromiso relevante. Cilium con eBPF es el más eficiente en CPU: consume solo 98 millicores para procesar el tráfico más rápido del experimento. Su métrica de eficiencia (9.84 Gbps por cada 100 millicores de CPU) prácticamente duplica la de Flannel (4.92 Gbps / 100\,m). Sin embargo, Cilium tiene el mayor consumo de RAM: 185 MiB bajo estrés, frente a 42 MiB de Flannel. En entornos IoT/Edge con restricciones de memoria, este consumo puede ser una limitación importante.
...
La Tabla~\ref{tab:normalizacion-validacion} muestra los valores brutos de cada métrica junto con su equivalente normalizado en la escala 0--1 que utiliza el modelo MCDA. La regla de lectura es uniforme: \textbf{1.00 = mejor CNI en esa dimensión; 0.00 = peor CNI en esa dimensión}, independientemente de las unidades originales (Gbps, ms, millicores). Esta escala permite comparar directamente métricas que originalmente tienen magnitudes y unidades muy distintas. En métricas donde un valor mayor es deseable (throughput), el CNI con el Gbps más alto recibe 1.00; en métricas donde un valor menor es deseable (latencia, CPU, RAM), el CNI con el valor más bajo recibe 1.00.

\begin{table}[htbp]
\centering
\scriptsize
\caption{Validación de la normalización min-max de las seis métricas del modelo MCDA. Escala normalizada: 1.00 = mejor, 0.00 = peor en cada dimensión.}
...
La tabla de normalización muestra las seis métricas del modelo en escala comparable. Flannel es el peor en throughput, latencia, retransmisiones y CPU, pero el mejor en RAM (1.00). Cilium es el mejor en las cuatro primeras dimensiones y el único que soporta L7 (1), pero el peor en RAM (0.00). Este compromiso multidimensional es precisamente lo que justifica el enfoque MCDA con perfiles: ningún CNI domina en todas las dimensiones de forma simultánea.

\subsection{Validación del Scoring MCDA por Perfil}

Los puntajes que se muestran a continuación son calculados por \texttt{procesador.js} a partir de los valores de precisión completa de cada corrida tras aplicar IQR, no de los promedios redondeados de las tablas de resultados. Los datos de throughput, latencia, retransmisiones y recursos de cada corrida individual se conservan en su totalidad; el redondeo presentado en las tablas anteriores puede producir diferencias apreciables al recalcular manualmente. En la Tabla~\ref{tab:scores-mcda}, \textbf{el puntaje va de 0 a 1: cuanto mayor el valor, más adecuado es ese CNI para ese perfil}. El símbolo~$\star$ marca la recomendación del sistema para cada perfil. Un puntaje de 0.834 significa, por ejemplo, que ese CNI ocupa el 83.4\% del espacio posible entre el peor y el mejor CNI teórico para ese conjunto de pesos.

\begin{table}[htbp]
\centering
\scriptsize
\caption{Puntajes MCDA finales por CNI y perfil ($\star$ indica recomendación; escala 0--1, mayor es mejor).}
...
Los resultados son coherentes con las expectativas técnicas. Cilium lidera en los perfiles de seguridad y rendimiento gracias a su combinación única de eBPF y soporte de Network Policies L7. Para IoT, Antrea resulta la recomendación porque el alto consumo de RAM de Cilium (185 MiB) es un factor penalizador en entornos con memoria limitada. Este resultado demuestra que el modelo MCDA captura correctamente los compromisos entre CNIs según el contexto.
