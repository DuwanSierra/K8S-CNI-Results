...
Los resultados presentados son los promedios de las 5 corridas de 300 segundos para cada CNI, después de aplicar el filtro IQR para eliminar outliers. La regla de agregación es única para los cuatro plugins y para todas las métricas ---promedio de las corridas supervivientes al descarte por rango intercuartílico, sin excepciones ni selección manual de corridas---, de modo que las corridas efectivamente descartadas varían entre plugins únicamente como consecuencia de aplicar el mismo criterio a distribuciones distintas. Las filas de outliers descartados de la Tabla~\ref{tab:throughput-resultados} documentan ese resultado: el descarte de una corrida de Calico en retransmisiones y de una de Cilium en throughput responde al mismo umbral estadístico, no a un tratamiento diferenciado. La tabla registra tres dimensiones de calidad del canal: cuántos datos por segundo puede transportar (throughput), con qué frecuencia debe reenviar paquetes (retransmisiones) y qué tan predecible es el canal entre corridas (variabilidad).
...
\hline
Retransmisiones TCP --- paquetes/sesión \newline\textit{(conteo bruto; no comparable entre CNIs)} & 11774 & 68993 & 15142 & 42229 \\
\hline
Retransmisiones por GB transferido \newline\textit{($\downarrow$ menor es mejor: tasa comparable entre CNIs)} & \textbf{212.0} & 1021.2 & 441.7 & 679.1 \\
\hline
Pérdida equivalente --- \% de los datos enviados & \textbf{0.029\%} & 0.145\% & 0.056\% & 0.112\% \\
\hline
...
El conteo bruto de retransmisiones requiere una precisión metodológica antes de ser interpretado. Las cinco corridas de cada plugin comparten una ventana idéntica de 300 segundos, pero no un volumen transferido idéntico: al sostener mayor caudal, Calico envió 67.7 GB por corrida frente a los 52.4 GB de Flannel, de manera que parte de su exceso de retransmisiones se explica por haber movido más datos y no por una deficiencia del plano de datos. Comparar los conteos absolutos penalizaría al plugin más rápido precisamente por serlo. Normalizada por volumen, la tasa ordena a los plugins como Flannel (212 por GB), Cilium (441.7), Antrea (679.1) y Calico (1021.2).

La ordenación se mantiene, pero su magnitud se relativiza: expresadas como fracción de los datos enviados, las cuatro tasas se sitúan entre el $0{,}029\%$ y el $0{,}145\%$, valores que TCP absorbe mediante retransmisión selectiva sin degradación perceptible para las aplicaciones. La diferencia es real y ordena a los plugins de forma consistente, pero ninguno de los cuatro exhibe un comportamiento que pueda calificarse de inestable. La \cref{sec:Discusion} analiza la relación entre caudal alcanzado y tasa de retransmisión.
...
\hline
Fintech / Seguridad crítica & 0.832 & 3.168 & $\star$ \textbf{3.521} & 2.186 \\
\hline
Streaming / Alto rendimiento & 3.133 & $\star$ \textbf{3.567} & 3.194 & 2.233 \\
\hline
IoT / Recursos limitados & $\star$ \textbf{3.259} & 2.952 & 3.148 & 2.205 \\
\hline
\end{tabularx}
\end{table}

Ningún CNI encabeza los tres perfiles: la recomendación se invierte según las prioridades declaradas. Este resultado ---y no la preferencia por un plugin concreto--- es el hallazgo central del modelo, y sostiene empíricamente que la selección de un CNI no admite una respuesta única.

Cilium lidera el perfil Fintech por su puntaje de seguridad (5.00), el único respaldado por aplicación efectiva de políticas de capa 7. Calico encabeza Streaming: aporta el mayor throughput medido (1804.40 Mbps) y la menor latencia de establecimiento, y el perfil solo declara una prioridad de seguridad baja, insuficiente para descartarlo. Flannel conserva el perfil IoT gracias a su huella de memoria (1176.54 MB) y CPU (19.45\%), decisiva en dispositivos con recursos muy limitados.

La ventaja de Flannel en IoT es, no obstante, estrecha: $3.259$ frente a $3.148$ de Cilium, poco más de una décima sobre una escala de $1$ a $5$. Esa diferencia queda dentro del margen que una variación de pesos o una nueva campaña de medición podría revertir, por lo que la recomendación para IoT debe leerse como una preferencia condicionada al ahorro de recursos y no como una superioridad establecida. La \cref{sec:limitaciones_mcda} desarrolla esta limitación.
...
El usuario ajusta cada deslizador de $1$ (Muy Bajo) a $5$ (Muy Alto); un campo de texto opcional permite además nombrar el tipo de proyecto. A medida que los deslizadores cambian, el motor en JavaScript calcula en tiempo real los pesos normalizados, evalúa la matriz MCDA contra los datos experimentales procesados de \path{cni-data.json} y reordena dinámicamente el ranking de CNIs. La aplicación no ofrece perfiles predefinidos seleccionables: los perfiles Fintech, Streaming e IoT documentados en la Tabla~\ref{tab:apb_pesos} son combinaciones de referencia de los mismos deslizadores, no botones de la interfaz.

\subsection{Asistencia de Despliegue e Instalación Guiada}

Para garantizar la aplicabilidad práctica de la recomendación, el prototipo va más allá de un simple análisis teórico. Junto al ranking, la aplicación presenta para el CNI recomendado un panel de despliegue con el comando de instalación del plugin y una política base de denegación por defecto, ambos copiables al portapapeles.
...
Por ejemplo, cuando el perfil regulado (prioridad de seguridad alta) sitúa a Cilium en el primer puesto, el recomendador entrega el comando \texttt{kubectl apply} del manifiesto de instalación de Cilium junto a la política \texttt{default-deny-ingress-egress} (Figura~\ref{fig:rec_fintech_instalacion}). Cuando el perfil de recursos limitados sitúa a Flannel en el primer puesto, entrega el manifiesto equivalente para desplegar el plugin en hardware restringido (Figura~\ref{fig:rec_iot_instalacion}). El panel no genera configuraciones avanzadas específicas por plugin ---por ejemplo, la habilitación de enrutamiento eBPF o de reglas L7 en Cilium---, que quedan a cargo del operador. Esta funcionalidad sitúa al prototipo como un punto de partida accionable para el aprovisionamiento de red, no como un instalador completo.
...
Para verificar que la SPA produce los rankings correctos ante distintas combinaciones de prioridades y que el motor MCDA opera sin pérdida de precisión numérica a lo largo del pipeline, se ejecutaron ocho casos de prueba funcionales y una traza de extremo a extremo sobre un dato real del laboratorio. Seis casos se aprobaron en su totalidad; PF-06 y PF-08 se verificaron con alcance acotado, según se detalla en la Tabla~\ref{tab:pruebas-funcionales-spa}.
...
\hline
PF-01: Prioridades del perfil Fintech/Seguridad en los deslizadores & ESPERADO: Cilium \#1 con score 3.521, Calico (3.168). OBSERVADO: Coincide. $\checkmark$ \\
\hline
PF-02: Prioridades del perfil Streaming/Rendimiento en los deslizadores & ESPERADO: Calico \#1 con 3.567, Cilium (3.194). OBSERVADO: Coincide. $\checkmark$ \\
\hline
PF-03: Prioridades del perfil IoT/Recursos en los deslizadores & ESPERADO: Flannel \#1 con 3.259, Cilium (3.148). OBSERVADO: Coincide. $\checkmark$ \\
\hline
PF-04: Usuario cambia las prioridades sin recargar la página & ESPERADO: El ranking se actualiza en menos de 50ms. OBSERVADO: Actualización $<$50ms. $\checkmark$ \\
\hline
...
\hline
PF-06: Incorporación de datos regenerados & ESPERADO: la SPA consume la versión vigente de \texttt{cni-data.json} sin servir copias en caché. OBSERVADO: la petición se emite con \texttt{cache: 'no-store'} y toma el archivo publicado en cada carga de la página. La aplicación no sondea el archivo durante la sesión: la actualización exige recargar. PARCIAL \\
\hline
...
\hline
PF-08: Manejo de error si \texttt{cni-data.json} no está disponible & ESPERADO: la aplicación sigue operativa ante un fallo de descarga. OBSERVADO: conmuta al conjunto de métricas embebidas y el indicador de la cabecera pasa a ``Datos de referencia''; no ejecuta reintento automático de la petición. PARCIAL \\
\hline
\end{tabularx}
\end{table}
...
\hline
OE2: Evaluar cómo la configuración segura de las Network Policies reduce la superficie de ataque. & CUMPLIDO PARCIALMENTE dentro del alcance operativo declarado (Apéndice~\ref{chap:apendice_oe2}, \S\ref{sec:apb2_alcance}). 8 casos de prueba de seguridad ejecutados en los 4 CNIs; Flannel no implementa Network Policies en su plano de datos; Cilium es el único con soporte L7. Overhead medido por escenario (Apéndice~\ref{chap:apendice_oe2}, \S\ref{sec:apb2_overhead}): variación de latencia entre $-43.16\%$ y $+31.45\%$; pérdida máxima de throughput $4.35\%$. La evidencia confirma el enforcement y el overhead, pero no cuantifica la probabilidad de incidentes de seguridad. Delimitación y comparación con marcos de referencia en \S\ref{sec:delimitacion_oe2} de la Discusión. \\
\hline
...
\hline
OE4: Implementar un prototipo funcional que recomiende el CNI más adecuado. & CUMPLIDO en su funcionalidad central --- 8 casos de prueba funcionales: 6 aprobados en su totalidad y 2 (PF-06 y PF-08) verificados con alcance acotado. Prueba de integridad de datos de extremo a extremo completada. El prototipo recomienda y entrega el comando de instalación y una política base; no automatiza la actualización de datos en sesión ni el reintento de descarga. \\
\hline
\end{tabularx}
\end{table}

\subsection{Matriz de Trazabilidad de Objetivos}

La \cref{tab:trazabilidad-objetivos} traza cada objetivo específico ---enunciado íntegramente en \cref{sec:Objetivos}--- hasta su evidencia documental y su evidencia en el repositorio o el aplicativo, junto con el estado alcanzado y las limitaciones que acotan su interpretación. La matriz permite verificar el cumplimiento contra fuentes reproducibles y no solo contra la narrativa del documento.

\begin{table}[htbp]
\centering
\scriptsize
\renewcommand{\arraystretch}{1.2}
\caption{Matriz de trazabilidad de los objetivos específicos con evidencia documental y del repositorio.}
\label{tab:trazabilidad-objetivos}
\begin{tabularx}{\textwidth}{|p{0.9cm}|Y|Y|p{1.6cm}|Y|}
\hline
\textbf{OE} & \textbf{Evidencia en el documento} & \textbf{Evidencia en el repositorio / aplicativo} & \textbf{Estado} & \textbf{Observación o limitación} \\
\hline
OE1 & Tablas de throughput, latencia y consumo de recursos (\cref{sec:Resultados}); protocolos de prueba con iperf3. & \path{results/cni-benchmarks/*/throughput_tcp}, \path{latency_tcp_connect} y \path{resource_usage_nodes} para los cuatro CNI. & CUMPLIDO & La métrica de RAM arrastra el ruido de los \emph{buffers} de red del host; la tendencia relativa se mantiene válida. \\
\hline
OE2 & Pruebas de seguridad con Network Policies (\cref{sec:Resultados}); Apéndice~\ref{chap:apendice_oe2}. & \path{K8s-Tesis-Apps/network_policies}: casos \path{use-case-1-zero-trust}, \path{use-case-2-multi-tier} y \path{use-case-3-egress-block}. & CUMPLIDO PARCIALMENTE & Flannel no implementa \emph{enforcement} de Network Policies en su plano de datos; la evidencia valida el control de comunicaciones y el overhead, pero no cuantifica la probabilidad de incidentes. El mapeo a marcos de referencia es conceptual. \\
\hline
OE3 & Criterios, umbrales y modelo MCDA (\cref{sec:apb_criterios,sec:apb_mcda}); Apéndice~\ref{chap:apendice_oe3}. & \path{docs/procesador.js} (IQR, min-max, scoring) y \path{results/processed/thesis_data.json}. & CUMPLIDO & Los umbrales de normalización min-max dependen del rango observado en el testbed; recalculan al incorporar nuevas corridas. \\
\hline
OE4 & Validación del prototipo y pruebas PF-01 a PF-08 (\cref{sec:prototipo_recomendador}); manuales (Anexos~\ref{anexo:manual_usuario} y~\ref{anexo:manual_instalacion}). & \path{cni-recommender-spa/src/data/recommendationModel.js}, \path{docs/recommender/cni-data.json} y la SPA publicada. & CUMPLIDO & La recomendación se validó en entorno de prueba; no constituye certificación para producción. La configuración que entrega el prototipo se limita al comando de instalación del plugin y a una política de denegación por defecto: no genera ajustes avanzados por plugin ni ejecuta el despliegue. \\
\hline
\end{tabularx}
\end{table}
